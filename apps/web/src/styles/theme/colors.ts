export const baseColors = {
	...
	alabaster: (opacity: number = 1) => `rgba(249, 249, 249, ${opacity})`,
	teal: (opacity: number = 1) => `rgba(0, 128, 128, ${opacity})`
} as const;

export const colors = {
	bg: {
		lightPrimary: (opacity: number = 1) => baseColors.alabaster(opacity),
	...
	border: {
		primaryRgb: (opacity: number = 1) => `rgba(0, 0, 0, ${opacity})`,
		secondaryRgb: (opacity: number = 1) => `rgba(0, 126, 128, ${opacity}`
	} as const,
