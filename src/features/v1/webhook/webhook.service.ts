...
export const updateStravaActivityWithLLMDescription = async (
...
export const getHeartRateForEachSongFromActivity = async (
  ...
  const endActivityDate = incrementDateBySeconds(
    elapsedTime,
    new Date(startActivityDate),
  );

  const songs = await fetchSongsByActivity(
    spotifyAccessToken,
    startActivityDate,
    endActivityDate,
  );
  ...
  const songsWithHeartRateData = heartRateTimeRanges.reduce(
    (prev, cur, index) => {
      prev.push({
        dataHR: dataHR.data.slice(
          index === 0 ? cur[0] : cur[0] + 1,
          cur[1] + 1,
        ),
