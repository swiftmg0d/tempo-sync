...
import { STRAVA_CLIENT_ID, STRAVA_CLIENT_SECRET, VERIFY_TOKEN, WEBHOOK_CALLBACK_URL } from '@/config/env';
import { STRAVA_WEBHOOK_URL } from '@/constants';
import { LLMActivityInsightResponse, LLMHeartbeatAnalysisResponse } from '@/types/llm.type';
...
import { getHeartRateForEachSongFromActivity, updateStravaActivityWithLLMDescription } from './webhook.service';
...
export const verifyStravaWebhook = (req: Request, res: Response) => {
	...
	res.json({ message: 'No mode or token' }).status(400);
};
...
export const stravaWebhookResponse = async (req: Request<unknown, unknown, WebhookRequestBody>, res: Response) => {
	...
	const updatedActivity = await updateStravaActivityWithLLMDescription(activity, stravaAccessToken);
	...
	const heartBeatAnalysis = await generetePrompt<LLMHeartbeatAnalysisResponse>(heartRateData, 'heartbeatSongAnalysis');

	let activityInsight = undefined;

	if (updatedActivity.map.polyline !== '') {
		activityInsight = await generetePrompt<LLMActivityInsightResponse>(updatedActivity, 'stravaInsight');
	}

	const response = await saveActivity(updatedActivity, athleteId, heartBeatAnalysis, activityInsight);
...
export const deleteWebhook = async (req: Request, res: Response) => {
	...
	const response = await stravaAPI({ baseURL: STRAVA_WEBHOOK_URL }).delete(`/${id}`, {
		params,
	});
