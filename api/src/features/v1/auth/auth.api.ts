...
import { REDIRECT_URL, STRAVA_CLIENT_ID, STRAVA_CLIENT_SECRET } from '@/config/env';
...
export const fetchSpotifyAccessToken = async (code: string): Promise<SpotifyAuthResponse> => {
	try {
...
export const fetchStravaAcessToken = async (code: string): Promise<StravaAuthResponse> => {
	try {
		const params = new URLSearchParams();
		...
		const { data } = await stravaAPI({ baseURL: STRAVA_TOKEN_URL }).post<unknown, { data: StravaAuthResponse }>('', params);
